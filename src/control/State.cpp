...
std::shared_ptr<Hw::KinovaArm> State::JacoZED = nullptr;
...
void State::init(std::shared_ptr<Hw::KinovaArm> jacoZed) {
	JacoZED = jacoZed;
}
...
void StateChangeMode::entryActionHook() {
	JacoZED->changeMode(static_cast<Hw::SteeringMode>(eventVariable));
}
...
void StateChangeModeDefault::entryActionHook() {
	JacoZED->changeMode(static_cast<Hw::SteeringMode>(0));
}
...
void StateMovePosition::entryActionHook() {
	if (!isKnownObjective(eventVariable)) {
		throw std::invalid_argument{"The supplied event variable was no objective ID!"};
	}
	JacoZED->setTarget(static_cast<ObjectiveId>(eventVariable));
}
...
void StateTeach::entryActionHook() {
	...
	JacoZED->teachPosition(static_cast<ObjectiveId>(eventVariable));
}
