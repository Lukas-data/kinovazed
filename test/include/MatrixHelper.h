...
struct MatrixComparisonWrapper {
	explicit MatrixComparisonWrapper(f2d_vec_t data)
	    : data{data} {
	}
	f2d_vec_t data;
...
struct VectorComparisonWrapper {
	explicit VectorComparisonWrapper(std::vector<float> data)
	    : data{data} {
	}
	template<std::size_t N>
	explicit VectorComparisonWrapper(std::array<float, N> data)
